         ...
         TIME: do k = 1, kmax ! this is the main time loop, central piece of this code!!!
            ...
            do i=1, nmax
               do j=1, nmax
                  ...
                  if( iter.ne.0.and.k.le.kmin ) then ! special case exception for non-zero stage iterations
                     ...
                     if( iv(i,j).eq.0 ) then ! this is the main velocity update, core of the code! if cell is assigned as locked...
                        ...
                        if( Dc(i,j).eq.0.0d0.or.(const*p000 + a(i,j)*dt).ge.0. ) then ! if this criterion holds (ask Hideo. Cell reached residual strength?),
                           ...
                           vel(i,j,k)  = (dsigma - dtau)/(const*p000 + a(i,j)*dt) ! what does the term a(i,j)*dt mean physically? Ask Hideo.
                           if((w(i,j)+vel(i,j,k)*dt).gt.dc(i,j)) then ! if accumulated slip at position larger than critical slip distance...
                              vel(i,j,k) = (tr(i,j) - dtau)/(const*p000) ! slip velo is residual stress - driving stress / unit factor
                           endif
                        endif
