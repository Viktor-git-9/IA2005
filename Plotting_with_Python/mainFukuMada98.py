...
runName = "run1" # name of selected run
...
slipVelocities = run.load("slipVelocities", scaleIndex, timeIndex)
...
scaleIndex2 = 0
plotTimeSteps = [0, 10, 20, 30, 40, 50, 60, 70]
...
for iTime in plotTimeSteps:
    ...
    temp_slipVelos.append(run.load("slipVelocities", scaleIndex2, iTime))
...
plotContours(temp_slip, timeStepLabels, cbarLabels2, globalTitle="Acc. Slip")
plotContours(temp_slipVelos, timeStepLabels, cbarLabels2, globalTitle="Slip velos")
...
axesLabels = ["y [element count]", "Stress [MPa]"]
